...
static void draw(GtkApplication *const app){
  ...
  gtk_window_set_title(GTK_WINDOW(window),"Qu}9^/");
...
int main(const int argc,char **const argv){
  ...
  if(!gtk_init_check()){
    printf("falling back to textual interface\n");
    return 0;
  }
  ...
  // (B) Command line args e.g. --gapplication-app-id
